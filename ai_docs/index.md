...
| `openrouteservice.md`  | Contrat de l'API ORS Directions (⚠️ ordre `[lon, lat]`), client serveur, Route Handler proxy, schéma Zod de réponse.                                              | Au moment d'implémenter/modifier `app/api/route/route.ts` ou `lib/ors.ts`.   |
